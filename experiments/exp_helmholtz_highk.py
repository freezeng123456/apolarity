...
def make_aniso(a1=1, a2=4, k=1.0):
    ...
    u = sin(a1 pi x) sin(a2 pi y), q = (k^2 - (a1 pi)^2 - (a2 pi)^2) u.  The
    directional anisotropy (a2 >> a1) is the original gradient-pathology case."""
    kappa2 = k ** 2
    S = (a1 ** 2 + a2 ** 2) * math.pi ** 2          # -Delta eigenvalue
    lam = kappa2 - S                                # source multiplier
    u = _u_aniso(a1, a2)
    fmax = max(a1, a2)
    return [LinearProblem(
        name=f"helm_aniso_{a1}_{a2}", d=2, order=2,
        terms=laplacian_power_terms(2, 1), zeroth=kappa2,
        u_exact=u, source_f=(lambda u=u, lam=lam: (lambda x: lam * u(x)))(),
        res_scale=abs(lam), S=S, bc_lap_powers=(), sweep=float(fmax),
