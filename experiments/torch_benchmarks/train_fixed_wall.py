...
def parse_args(argv=None):
    ...
    p.add_argument('--expected-device',choices=('T4','V100','RTX 3080'),default='RTX 3080')
    p.add_argument('--eval-seed-base',type=int,default=20261400,help='Independent evaluation stream; offsets 1,2,3,99.')
    ...
    p.add_argument('--trajectory-seconds',type=float,default=2,
                   help='Optional model-only snapshots for offline error curves; 0 disables.')
    p.add_argument('--eval-points',type=int,default=10000)
    p.add_argument('--constraint-sampling',choices=('resampled',),default='resampled')
    p.add_argument('--constraint-side',type=int,default=0)
    p.add_argument('--max-steps',type=int,default=0,help='smoke checks only')
    a=p.parse_args(argv)
    if a.lr is None:a.lr=1e-4 if a.case=='ch2d' else 1e-5
    if not all(math.isfinite(v) for v in (a.seconds,a.lr,a.checkpoint_seconds)):p.error('finite settings required')
    if a.depth < 2 or a.max_steps < 0 or (a.constraint_side and a.constraint_side < 2):p.error('invalid depth, step cap or constraint count')
...
def full_loss(module,model,points,data,method,independent=False):
    ...
    if module is problem_ch2d:return module.loss(model,points,data,method,independent=independent)
...
def train(a):
    if a.constraint_sampling != 'resampled':raise ValueError('paper training requires resampled constraints')
    ...
    with (a.out/'steps.jsonl').open('w',buffering=1) as stream:
        while time.perf_counter()-started<a.seconds:
            ...
            constraint_hashes.append(batch_hash(constraint_np))
    ...
    save(a.out/'summary.json',dict(case=a.case,method=a.method,steps=step,wall_seconds=wall,
         ...
         initial_hash=initial_hash,final_hash=final_hash,constraint_sampling="resampled",errors=metrics,final_fixed_loss=curve[-1]['fixed_loss'],
