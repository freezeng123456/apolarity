...
def main() -> None:
    ...
    assert manifest['expected_cells'] == 20 and manifest['expected_rows'] == 60 and manifest['expected_timed_calls'] == 1800
    ...
    for target, method, batch in sorted(expected_cells, key=lambda item: (item[2], len(item[0]), item[0], item[1])):
        ...
        for record in records:
            ...
            rows.append(record)
    if len(rows) != 60 or not source_hash_ok:
    ...
    audit = {
        ...
        'expected_cells': 20, 'observed_cells': len(expected_cells),
        'expected_records': 60, 'observed_records': len(rows),
        'expected_timed_calls': 1800, 'observed_timed_calls': sum(len(row['times_ms']) for row in rows),
    ...
    report = [
        ...
        f"- 覆盖：{len(expected_cells)}/20 单元，{len(rows)}/60 种子记录，{sum(len(row['times_ms']) for row in rows)}/1800 次正式计时；预计和实际完全一致。", '',
        '## 正式矩阵', '',
    ...
    report += ['', 'The complete seed medians and paired ratios are retained in summary.csv.',
               'All derivative outputs are checked against nested coordinate JVPs.',
               'Direction counts alone do not determine measured runtime.']
    ...
    readme = [
        ...
        '- `summary.csv`：保留三种子中位数与配对比值的机器可读表。',
