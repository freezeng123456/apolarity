"""Isolated g-KdV linear reconstruction timing, with the original timing policy unchanged."""
import argparse
import hashlib
import os
...
from jax_apolarity_bench.gkdv_optimized import METHODS, make_gkdv_evaluator
...
def verify(args):
    ...
    for seed in args.seeds:
        params, points = inputs(seed, 'gkdv1d', 3, args.width, args.depth)
        expected, components = independent_reference('gkdv1d', params, points)
        jax.block_until_ready((expected, components))
        for method in METHODS:
            value, d = make_gkdv_evaluator(method, components=True)(params, points)
            row = dict(method=method, seed=seed, value=check(value, expected),
                       components={k: check(d[k], v) for k, v in components.items()})
            check(make_gkdv_evaluator(method)(params, points), expected)
            rows.append(row)
...
def benchmark(args, device):
    evaluate = make_gkdv_evaluator(args.method)
    params, points = inputs(args.seeds[0], 'gkdv1d', args.batch, args.width, args.depth)
    start = time.perf_counter()
    jax.block_until_ready(evaluate(params, points))
    cold_s = time.perf_counter() - start
    rows = []
    for seed in args.seeds:
        params, points = inputs(seed, 'gkdv1d', args.batch, args.width, args.depth)
        ...
        assert value.shape == () and value.dtype == np.float32 and np.isfinite(value).all()
        stats = device.memory_stats() or {}
        row = dict(case='gkdv1d', method=args.method, batch=args.batch, seed=seed,
        ...
        checked_value, components = make_gkdv_evaluator(args.method, components=True)(params, points)
...
def main():
    ...
    try:
        ...
        sources = {str(f.relative_to(HERE)): hashlib.sha256(f.read_bytes()).hexdigest()
                   for f in sorted([*HERE.glob('*.py'), HERE / 'GKDV_LINEAR.md', *HERE.joinpath('src').rglob('*.py')])}
        save(args.out / 'config.json', {**vars(args), 'out': str(args.out),
             'case': 'gkdv1d', 'protocol': 'gkdv_linear_v1',
             ...
             'parameters_and_inputs_dynamic': True, 'task': 'complete_gradient_enhanced_scalar_objective',
             ...
             'schedule': {'directions': 5, 'jets_by_order': {'4': 5}} if args.method in METHODS[2:] else None,
