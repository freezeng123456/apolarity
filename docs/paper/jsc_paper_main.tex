...
\begin{abstract}
  ...
  minimum. Experiments on fixed high-order partial derivatives assess the
...
\item We compare the WDD method with nested coordinate Jacobian--vector
  products (JVPs) on fixed high-order partial derivatives, examining runtime
...
fixed high-order partial derivatives and examines its use in complete
...
isolated high-order partial derivatives across various orders and
multi-index patterns, using the minimum-direction complex Waring
representation in Corollary~\ref{thm_main}. Second, we assess end-to-end
PINN training performance on three nonlinear differential equations:
a one-dimensional KdV equation with a gradient-enhanced objective,
a two-dimensional KdV equation with mixed derivatives, and a fourth-order
Cahn--Hilliard equation. For these PDE problems, we apply
Algorithm~\ref{alg_homogeneous} to real power-sum identities, which also
allow lower-order Taylor coefficients to be reused across different
derivative terms. In every paired comparison, the WDD method and the
nested coordinate Jacobian--vector product (JVP) baseline compute the
same mathematical derivatives; in the PDE experiments, they also minimize
identical loss objectives under identical computational settings.

For the fixed-partial tests, the network has 16 inputs and three
width-128 $\tanh$ hidden layers. Both methods use identical weights and
input points. For each of three seeds, 10 warm-up calls precede 30
synchronized timed calls. Times are reported in milliseconds as the mean
and sample standard deviation of the three per-seed medians; speedups
average the paired ratios. These timings cover derivative-value evaluation
without parameter backpropagation.

In the PDE examples, $x$ and $y$ denote spatial coordinates and $t$
denotes physical time. Initial data are imposed at $t=0$, and terminal-time
errors are evaluated at $t=1$. The vector $\bm n$ denotes the outward
spatial unit normal.
...
\subsection{Fixed high-order partial derivatives}\label{sec_num_problems}

This experiment examines how derivative order, multi-index pattern,
and input batch size affect evaluation time.
We write $u_{i_1\cdots i_p}=\partial_{i_1}\cdots\partial_{i_p}u$ and evaluate
the six targets in Table~\ref{tab_fixed_t4}. Here $p$ is the derivative
order, $R$ the minimum complex direction count, and $B$ the number of
input points.

The first four targets have order six and exponent patterns $(6)$,
$(3,3)$, $(2,2,2)$, and $(1,1,1,1,1,1)$, respectively. They compare a pure
partial with mixed partials involving two, three, and six coordinates at
the same order; their minimum complex direction counts are $1$, $4$, $9$,
and $32$. The last two targets, with patterns $(2,2,2,2)$ and $(3,3,3)$,
test orders eight and nine, with $27$ and $16$ directions. All counts follow
from the monomial rank formula in Section~\ref{sec_rank_optimal}.
The paired outputs agree within floating-point
tolerances, supporting the numerical realization of the exact directional
formulas.
...
\caption{Evaluation times and speedups for fixed high-order partial derivatives.}
\label{tab_fixed_t4}
\input{tables/t4_fixed_partials}
\end{table}

Table~\ref{tab_fixed_t4} reports the evaluation times and speedups.
The WDD method reduces evaluation time for every target.
At $B=100$, the sixth-order timings remain close despite the different direction
counts, indicating that representation length alone does not determine
batched runtime. In particular, the pure partial already has $R=1$;
its speedup measures the benefit of Taylor propagation relative to nested
JVPs, rather than a reduction in the number of directions.

For $u_{123456}$ and $u_{111222333}$, the table also compares input
batches of 100, 400, and 1600 points. The advantage over nested JVP
persists at the larger batches.
The following experiments examine whether this saving carries over to
complete PINN training.

\FloatBarrier

\subsection{Gradient-enhanced PINN for the KdV equation}\label{sec_num_pinn}
...
\input{tables/resampled_wall_kdv1d}
\end{table}
...
\subsection{Mixed-derivative PINN for a two-dimensional KdV equation}
\label{sec_num_kdv2d}
...
\input{tables/resampled_wall_kdv2d}
\end{table}
...
\subsection{Fourth-order Cahn--Hilliard equation}\label{sec_num_ch}
...
\begin{equation}\label{eq_num_ch}
 ...
 \end{cases}
\end{equation}
Here $\nabla$ and $\Delta$ act on the spatial coordinates.
...
\eqref{eq_num_ch}.}
\label{tab_wall_ch2d}
\input{tables/resampled_wall_ch2d}
\end{table}
...
The experiments show faster fixed-partial evaluation and more PINN updates
