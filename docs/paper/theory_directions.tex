...
\begin{remark}[The bound is not always attained over $\R$]\label{rmk_realgap}
  ...
  over real nodes and weights, described in \cref{sec_numerics}, reaches
  $D_{m,N}$ for $(N,m) = (3,1)$, $(3,2)$ and $(4,1)$, but for $(N,m) = (3,3)$
  the smallest real rule found has $11$ nodes against the bound $10$, and for
  $(N,m) = (4,2)$ it has $12$ nodes against the bound $10$ and requires weights
  of both signs.  This is consistent with the general fact that real Waring rank
  can exceed complex Waring rank, and with \cref{thm_orbit}: in degree $6$ the
  icosahedral group has a two-dimensional invariant space, so a single orbit no
  longer suffices.
\end{remark}
...
schedule found by numerical search.}
\label{tab_numbers}
\centering
...
$\Delta^{2}$ & $4$ & $125$ & $22$  & $10$ & $12$ & mixed sign \\
\midrule
