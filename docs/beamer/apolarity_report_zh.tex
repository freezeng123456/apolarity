...
\begin{frame}{PINN 实验：四维六阶 Cahn--Hilliard 类方程}
  ...
  \medskip
  \textbf{网络与损失.}\;
  $4$ 层 Linear--$\sinh$ 多层感知机，宽度 $64$，参数取值于 $\C^{\,\cdot}$（\texttt{torch.complex128}）。
  以 $\Re u$ 为物理解，损失项为
  \[
  ...
  \medskip
  \textbf{对照设置.}\;
  Tesla T4，每个 backend 给定相同的 wall-clock 预算 $60$ 秒，
  内点采样 $128$、边界采样 $64$，Adam（学习率 $10^{-3}$）。
  三个 backend 在同一损失、同一初始化、同一采样下做对照。
\end{frame}

\begin{frame}{PINN 实验结果}
  \footnotesize
  \begin{tabular}{lrrrrr}
    \toprule
    \textbf{backend} & \textbf{steps} & \textbf{ms/step} & \textbf{显存峰值 (MB)} & \textbf{最终 $L_{\mathrm{int}}$} & \textbf{相对 $L^2$ 误差} \\
    \midrule
    waring complex jet  & $4248$ & \alert{$14.1$} & \alert{$40$}  & \alert{$1.74\!\times\!10^{-3}$} & \alert{$8.50\!\times\!10^{-3}$} \\
    polarization jet    & $4192$ & $14.3$         & $49$          & $6.32\!\times\!10^{-3}$ & $4.43\!\times\!10^{-2}$ \\
    嵌套自动求导        & $211$  & $285.6$        & $103$         & $2.82\!\times\!10^{-1}$ & $2.49\!\times\!10^{-1}$ \\
    \bottomrule
  \end{tabular}
  ...
  \begin{itemize}\setlength\itemsep{1pt}
    \item 两条 jet 路径单步耗时为嵌套自动求导的约 $1/20$，显存约 $1/2.5$；
        $60$ 秒预算下嵌套自动求导仅完成 $211$ 步，$L^2$ 误差停在 $0.25$ 量级；
    \item 复 Waring 路径达到 $R_\C=5$，比实极化方向 $7$ 严格少 $2$ 个方向，
        相同时间内的最终 $L^2$ 误差领先约 $5$ 倍（$8.5\!\times\!10^{-3}$ 对 $4.4\!\times\!10^{-2}$）；
    \item $\sinh$ 在 $\C$ 上为整函数，复方向 jet 与复参数模型可直接耦合，无需逐步类型转换。
  \end{itemize}
\end{frame}
...
\begin{frame}{结论与下阶段工作}
  ...
  \begin{itemize}
    ...
    \item 在含重复指标的中高阶情形下，相比嵌套自动求导有显著加速。
  \end{itemize}
