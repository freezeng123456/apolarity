...
from train_kdv2d import parse_args, train
...
def test_training_eval_cadence_isolation(tmp_path, method):
    states = []
    for cadence in (1, 3):
        out = tmp_path/f'cadence_{cadence}'
        out.mkdir()
        a = parse_args(['--out',str(out),'--method',method,'--width','3','--depth','2',
                       '--batch','3','--constraint-side','2','--steps','3','--decay-steps','3',
                       '--eval-every',str(cadence),'--val-nxy','2','--val-nt','2','--test-nxy','2','--test-nt','2'])
        train(a)
        states.append(torch.load(out/'final.pt',map_location='cpu',weights_only=False))
    ...
    for key,state in states[0]['optimizer']['state'].items():
        for field,value in state.items():
            torch.testing.assert_close(value,states[1]['optimizer']['state'][key][field],rtol=0,atol=0)
    assert states[0]['optimizer']['param_groups'][0]['lr'] == 0


def test_sampling_and_cli():
    ...
    with pytest.raises(SystemExit):
        parse_args(['--out','unused','--method','nested_jvp','--steps','10001'])
