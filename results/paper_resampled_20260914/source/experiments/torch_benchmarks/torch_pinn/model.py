...
def sample_points(seed, n, dim, *, dtype=torch.float32, device='cpu'):
    """Same radius/time sampling as the historical PDE compute benchmark."""
    rng = np.random.default_rng(seed)
    radius = rng.uniform(0, 1, (n, 1))
    direction = rng.standard_normal((n, dim-1))
    space = direction/np.maximum(np.linalg.norm(direction, axis=-1, keepdims=True), 1e-12)*radius
    points = np.concatenate([space, rng.uniform(0, 1, (n, 1))], axis=-1)
...
def sample_normal(seed, n, dim, *, dtype=torch.float32, device='cpu', std=.35):
    ...
    space-time sampler for the PINN experiments, while the fixed mixed-partial
