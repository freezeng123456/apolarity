...
from .operators import partial, chain, selected
...
def components(model, points, method):
    if method == 'shared_jet_linear':
    ...
    elif method in ('nested_jvp','independent'):
...
def loss(model, points, data, method):
    d = components(model,points,method)
