#!/usr/bin/env python3
"""Run the approved fixed-weight Polyharmonic WAR/real-AD experiment.

The formal protocol is intentionally separate from the exhaustive 60-second
weight search.  It evaluates the three selected Poly task/weight pairs with five
...
Run or resume the approved 1200-second experiment::
    ...
    python scripts/run_poly_fixed_weight_formal.py orchestrate --seconds 1200 \
        --seeds 5 --resume
"""
...
DEFAULT_ROOT = (
    ROOT / "outputs" / "current" / "polyharmonic-common-xavier-fp32-formal-v1"
)
TASK_ORDER = (
    "poly_d2_o2",
    "poly_d2_o4",
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="apolarity-poly-formal-smoke-") as raw:
        raw_root = Path(raw)
        for task in tasks:
            for method in METHODS:
                output = raw_root / task / "seed_000" / f"{method}.json"
                log = raw_root / task / "seed_000" / f"{method}.log"
                ...
                print(
                    f"[smoke] {task} {method} weights={FIXED_WEIGHTS[task]}",
                    flush=True,
                )
